...
*/
  ...
  */
    @Get("/getMyData")
    public async getMyData(): Promise<IResponse> {
        try {
            //   check for a valid id
            // const getResponse = await getById(adminModel, this.userId);

            const getResponse = await adminModel.aggregate([
                // {
                //     $match: {
                //         $and:[
                //             { createdAt: { $gt: new Date(`${'2022-11-12'}T00:00:00Z`) , $lt: new Date(`${'2022-11-15'}T00:00:00Z`)} },
                //             {firstName : 'Admin'}
                //         ],
                //         // $or:[
                //         //     { createdAt: { $gt: new Date(`${'2022-11-12'}T00:00:00Z`) , $lt: new Date(`${'2022-11-15'}T00:00:00Z`)} },
...

                {
                    $group: {
                        ...
                        nameExists: {
                            $push: {
                                uName: '$firstName', lName: '$lastName', uEmail: '$email', date: '$createdAt'
                            },
                        },
                        itemsSold: { $addToSet: "$firstName" },
                        count: { $count: {} },
                        objdata: {
                            $push: {
                                date: { $dayOfWeek: new Date(`${'2022-11-11'}T00:00:00Z`) }
                            }
                        }
                    }
                },
                { $sort: { 'nameExists.date': 1 } },
    ...
    /**
  * Get user getMyData with pagination
  */
    @Get("/pagination")
    public async pagination(@Query() pageNumber = 1, @Query() pageSize = 5): Promise<IResponse> {
        try {
            const getResponse = await adminModel.aggregate([
                {
